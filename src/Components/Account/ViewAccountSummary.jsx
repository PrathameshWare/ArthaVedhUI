...
const ViewAccountsSummary = () => {
  ...
  const allCustomers = Array.from({ length: 25 }, (_, index) => ({
    ...
    accountType: index % 2 === 0 ? "Savings" : "Checking",
    accountName: `Account ${index + 1}`,
    accountCurrency: index % 2 === 0 ? "USD" : "EUR",
  ...
  const pendingCustomers = Array.from({ length: 25 }, (_, index) => ({
    ...
    customerId: `CUST${index + 1}`,
    accountType: "Savings",
    accountName: `Pending Account ${index + 1}`,
    accountCurrency: "USD",
    status: "New",
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex-grow flex flex-col p-5 bg-gray-100">
        <h1 className="text-2xl font-bold text-gray-800 mb-4">Accounts</h1>
          ...
          <table className="min-w-full border border-white">
            <thead>
              <tr className="bg-blue-950 text-white">
                <th className="p-2 border border-white">Account Id</th>
                <th className="p-2 border border-white">Account Number</th>
                <th className="p-2 border border-white">Customer Id</th>
                <th className="p-2 border border-white">Account Type</th>
                <th className="p-2 border border-white">Account Name</th>
                <th className="p-2 border border-white">Account Currency</th>
            ...
            <tbody>
              {currentCustomers.map((account) => (
                <tr key={account.id} className="bg-white text-black">
                  <td className="p-2 border border-white">{account.id}</td>
                  ...
                  <td className="p-2 border border-white">{account.status}</td>
                  <td className="p-2 border border-white">
                    {account.pendingStatus}
                  </td>
                  <td className="p-2 border border-white">
                    {account.createDate}
                  </td>
...
export default ViewAccountsSummary;
